...
public class WebSocket extends EventDispatcher {
    ...
    private static var log:ILogger = Log.getLogger("net.gimite.websocket.WebSocket");
  ...
  private var fragmentsBufferIsText:Boolean = false;
  ...
  public function WebSocket(
      ...
      cookie:String, headers:String, proxyHost:String = null, proxyPort:int = 0) {
    this.id = id;
    this.url = url;
  ...
  public function send(stringOrByteArray:Object):int {
	  if (stringOrByteArray is ByteArray)
	  	return sendByteArray(stringOrByteArray as ByteArray); 
	  return sendString(stringOrByteArray as String);
  }
  
  private function sendByteArray(dataBytes:ByteArray):int {
	if (!dataBytes || !dataBytes.length)
		return 0;
	log.debug("send: {0}", dataBytes.length);
    if (readyState == OPEN) {
  ...
  private function sendString(encData:String):int {
	  if (!encData)
		  return 0;
	  var data:String;
	  try {
		  ...
		  log.error("SYNTAX_ERR: URIError in send()");
		  return 0;
	  }
	  log.debug("send: " + data);
  ...
  public function close(
    ...
    if (code != STATUS_NORMAL_CLOSURE &&
      ...
      log.error("Fail connection by {0}: code={1} reason={2}", origin, code, reason);
    }
    ...
    if (closeConnection) {
      ...
      readyState = CLOSED;
      if (fireErrorEvent) {
        dispatchEvent(new WebSocketEvent(WebSocketEvent.ERROR));
      } else {
            ...
            switch (frame.opcode) {
				  ...
				  if (fragmentsBuffer == null) {
					  ...
					  if (frame.fin) {
						  if (fragmentsBufferIsText) {
							  ...
							  fragmentsBuffer.position = 0;
							  this.dispatchEvent(new WebSocketEvent(WebSocketEvent.BINARY, fragmentsBuffer));							  
						  }
              ...
              case OPCODE_TEXT:
				if (frame.fin) {
	                ...
	                try {
	                  this.dispatchEvent(new WebSocketEvent(WebSocketEvent.TEXT, encodeURIComponent(data)));
	                } catch (ex:URIError) {
	                  close(1007, "URIError while encoding the received data.");
	                }
				} else {
					fragmentsBuffer = new ByteArray();
					fragmentsBuffer.writeBytes(frame.payload, 0, frame.payload.length);
					fragmentsBufferIsText = true;
				}
                break;
              case OPCODE_BINARY:
                if (frame.fin) {
					dispatchEvent(new WebSocketEvent(WebSocketEvent.BINARY, frame.payload));					
				} else {
					fragmentsBuffer = new ByteArray();
					fragmentsBuffer.writeBytes(frame.payload);
					fragmentsBufferIsText = false;
				}
              ...
              case OPCODE_CLOSE:
                ...
                log.debug("received closing frame");
  ...
  private function sendFrame(frame:WebSocketFrame):Boolean {
    ...
    try {
      ...
      log.error("Error while sending frame: {0}", ex.message);
  ...
  private function dispatchCloseEvent(wasClean:Boolean, code:int, reason:String):void {
    var event:WebSocketEvent = new WebSocketEvent(WebSocketEvent.CLOSE);
